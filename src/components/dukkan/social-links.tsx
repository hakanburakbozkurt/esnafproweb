...
function SocialIcon({
  ...
  return (
    <a
      ...
      aria-label={label}
      className={cn(
        "inline-flex items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 transition hover:border-emerald-300 hover:text-emerald-600",
        box
      )}
    >
      {children}
...
export function SocialLinks({
  ...
  const links = [
    ...
    tiktok && { href: tiktok, label: "TikTok", icon: "tiktok" },
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-2", className)}>
      {links.map((link) => (
        <SocialIcon key={link.icon} href={link.href} label={link.label} size={size}>
          {link.icon === "instagram" && (
