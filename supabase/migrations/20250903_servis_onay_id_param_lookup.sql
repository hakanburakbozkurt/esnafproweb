-- /servis-onay: id query param + technical_service.id (UUID) lookup
...
BEGIN
  IF normalized_token IS NULL OR length(normalized_token) < 6 THEN
  ...
  SELECT * INTO rec
  FROM public.technical_service
  WHERE lower(trim(coalesce(approval_token, ''))) = lower(normalized_token)
     OR service_id = normalized_token
     OR tracking_code = normalized_token
     OR (
       normalized_token ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
       AND id = normalized_token::uuid
     )
  ORDER BY
    CASE
      WHEN lower(trim(coalesce(approval_token, ''))) = lower(normalized_token) THEN 0
      WHEN service_id = normalized_token THEN 1
      WHEN tracking_code = normalized_token THEN 2
      ELSE 3
    END,
    created_at DESC
  LIMIT 1
